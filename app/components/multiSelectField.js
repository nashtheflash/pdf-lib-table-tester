...
export function MultiSelect({field, fieldDef, userPdfSettings, setUserPdfSettings}) {
  ...
  const handelSelection = ({ id }) => {
    ...
    console.log(fieldDef.options[id - 1])
  ...
  return (
    <Listbox value={selected} onChange={(index) => handelSelection(index)}>
      {({ open }) => (
        <>
          <Listbox.Label className="absolute z-20 -top-2 left-2 inline-block bg-white px-1 text-xs font-medium text-gray-900">{field}</Listbox.Label>
